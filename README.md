...
新闻分类使用原项目已训练的 `char_ngram_svm`，不是规则匹配或伪造输出。TF-IDF 和 LinearSVC 的参数以 float32 导出，在浏览器中计算。参数以512KiB分块加载，支持压缩、自动重试与完整性校验；旧浏览器可使用未压缩分块。决策分数不是概率，片段贡献不是因果解释。用户输入不发送给推理服务器，也不写入本地存储。
